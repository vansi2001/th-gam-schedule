jQuery(function ($) {
  // Lắng nghe sự kiện click trên các phần tử có thuộc tính data-url
  ...
  // Tải dữ liệu lịch khi trang đã sẵn sàng
  $.ajax({
  ...
  function put_game_schedule(responseData) {
    responseData.forEach((element) => {
      ...
      // Xử lý chế độ xem dạng lịch (table view)
      if (
      ...
      // Xử lý chế độ xem dạng danh sách (list view)
      if (
